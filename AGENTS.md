...
The Python project lives under `lftools_uv/`. Run commands from the
repository root using `uv`:

- Run tests before committing: `uv run pytest tests/`
- Run linting before committing: `uv run ruff check .`
- Run type checking: `uv run mypy lftools_uv`
- Run strict type checking: `uv run basedpyright`
